...
def Nvs1fool(**kwargs):
	...
	test_data = myData_Nvs1(
			filelists =opt.test_filelists,
			#transform =data_transforms['val'],
			transform =None,
                        scale = opt.cropscale,
			test = True,data_source = 'none',current_path=os.getcwd())
...
#				test = True,transform = data_transforms['test'])
	...
	auc = metric[2]
	xy_dic = metric[3]
	pickle.dump(xy_dic, open('result/Nvs1foolxy.pickle','wb'))	
	print('EER: {:.6f} TPR(1.0%): {:.6f} TPR(.5%): {:.6f} AUC: {:.8f}'.format(eer, tprs["TPR(1.%)"], tprs["TPR(.5%)"], auc),file=open('result/Nvs1fool.txt','a'))
